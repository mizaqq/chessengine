...
| `midgame_boards` | int | 0 | Game boards (after the puzzle boards) that start from FENs in `game_start_file`, full-game rules and ply cap |
| `game_start_file` | str | – | CSV with a `fen` column; `scripts/prepare_games.py` writes `data/games/midgame_starts.csv` |
| `max_plies` | int | null (default yaml: 120) | Training game boards end as a draw after this many plies (AlphaZero terminated over-long games as draws); not applied to puzzle boards or evaluation games |
...
**Start-position curriculum**: `round(puzzle_fraction * num_envs)` boards play short episodes from Lichess mate-in-one and mate-in-two positions (default yaml: 50/50). A mate-in-one episode ends after the mover's move; a mate-in-two episode ends after the mover's second move, and a first move that is not the verified forcing key move ends it at once as a miss (mate = win; anything else ends as `puzzle_miss` paying the mover `puzzle_miss_reward`). The sparse outcome reward is thus one or two decisions away and every episode is an attempt; the other boards play full games, cut to a draw at `max_plies`. Logs report `puzzle_attempts`, `puzzle_solved_rate` (plus `_m1` / `_m2` by depth), and entropy split into `_game` / `_puzzle` (reverse curriculum by start state; Florensa et al. 2017, Salimans & Chen 2018). Build the Lichess files with `python -m scripts.prepare_puzzles --train 50000 --keep-eval data/puzzles/mate_in_1_eval.csv` and `python -m scripts.prepare_puzzles --depth 2 --train 50000` (downloads the CC0 dump to `data/raw/`; depth 2 keeps only puzzles whose key move is verified to force mate); harvest positions from a checkpoint's own games with `python -m scripts.harvest_positions <ckpt_dir>`. Human-game pretraining (AlphaGo's first stage): `python -m scripts.prepare_games --month 2013-12 --min-elo 1500` builds `data/games/human_{train,eval}.npz` and `midgame_starts.csv` from the Lichess games database, `python -m scripts.pretrain_human --out DIR` trains the network on human moves (cross-entropy plus a 0.01-weight value loss), and `init_from: DIR` continues with PPO. Evaluate checkpoints with `python -m scripts.eval_checkpoint_puzzles <ckpt_dir>` and in-game with `python -m scripts.eval_games <ckpt_dir>`.
