"""Test that the training loop handles desynchronized environments.

When one env finishes a game and resets while others continue,
current_player values diverge across envs (mixed batch). This
previously caused:
  - Size mismatch in loss accumulation (variable-size filtered tensors)
  - "Backward through graph a second time" (shared computation graph)
"""
import torch
