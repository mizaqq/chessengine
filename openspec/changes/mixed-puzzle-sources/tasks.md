...
- [ ] 1.1 `prepare_puzzles.py --train 50000 --keep-eval`: exclude held-out ids, rewrite train only; verify eval file unchanged (hash) and train has 50,000 rows disjoint from eval
...
- [ ] 2.1 `MixedSampler` with weights; tests for equal weights, zero weight, determinism
- [ ] 2.2 Config: `puzzle_train_files` (list of file/weight), `puzzle_boards` override with range check; tests for precedence and errors
- [ ] 2.3 Multi-set evaluation: `puzzle_eval_files` mapping, prefixed keys, single-file path unchanged; tests
