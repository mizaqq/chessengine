...
- [x] 1.1 Add failing tests in `tests/envs/` that `EnvStep.material` is 0 after reset
...
- [x] 1.2 Replace the per-step difference with `material` in both vector envs and
      `EnvStep`; remove `reward`; update existing env tests that asserted on
      `reward`; verify `uv run pytest tests/envs` passes.

## 2. Shaped returns

- [x] 2.1 Rewrite `tests/model/test_returns.py`: keep the single-player and
...
- [x] 2.2 Change `StepRecord` (`potential_white` replaces `reward_white`), add
      ...
      passes.
- [x] 2.3 Update `_collect_rollout` to store the pre-move material and
      `run_chess_training` to pass the final observation's material and the scale;
      verify `uv run pytest tests/integration` passes.

## 3. Configuration

- [x] 3.1 Parse `shaping` and `shaping_scale` in `src/entrypoints/train.py` with
...
- [x] 3.2 Add `mean_terminal_return` (unshaped, white view, averaged over games that
...
- [x] 4.1 Run `uv run pytest -q` and confirm green; run
      `uv run python -m src.entrypoints.train --max-updates 5` and confirm it
      finishes without NaN.
- [x] 4.2 Update README reward description and the "Returns Computation" note in
...
- [x] 4.3 Add the AlphaZero reward source to `learning/RESOURCES.md` once fetched,
